...
\fancyhead[R]{User Manual v1.1.0}
\fancyfoot[C]{\thepage}
...
\begin{enumerate}[itemsep=3pt]
    \item \textbf{Course Selection} --- Browse and select from your Gradescope courses. Use \texttt{Tab} to focus the list, \texttt{Space} to toggle selection, then \texttt{Tab+Enter} to continue.
    \item \textbf{Assignment Selection} --- Pick the assignment to grade.
...
\begin{tipbox}
\textbf{Keyboard navigation:} \texttt{Tab} to move focus, \texttt{Space} to toggle checkboxes, \texttt{Enter} to activate, \texttt{Escape} to go back, \texttt{q} to quit. Selected items appear in \textbf{bold}, unchecked items in dim text.
\end{tipbox}
...
\begin{enumerate}[itemsep=3pt]
    \item \textbf{Login} --- Authenticate with your Gradescope credentials (stored in session for subsequent steps).
    \item \textbf{Select Assignment} --- Choose course, fetch assignments, pick one.
    \item \textbf{Configure} --- Upload question/answer PDFs, rubric YAML (auto-parsed), set extra instructions, pick provider (OpenCode Go / LM Studio) and model.
    \item \textbf{Grade} --- Run grading with live progress display and log output.
    \item \textbf{Export} --- Download results as Gradescope CSV, detailed CSV, or JSON.
\end{enumerate}
